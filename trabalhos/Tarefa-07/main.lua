...
-- TAREFA 07
-- ALOCAÇÃO: Neste momento os inimigos são alocados	
	table.insert(enemies, newEnemy)
	end

-- TAREFA 07
-- TEMPO DE VIDA: Desde sua alocação até sofrer alguma colisão (bala ou nave) ou até chegar 
-- na posição 600 do eixo y na tela.

-- TAREFA 07
-- ESCOPO: enemies está declarada na load deste programa, se comportando assim como um global, 
-- logo ela é visivel em toda a execução do programa e não apenas dentro desta função 
	for i, enemy in ipairs(enemies) do
		enemy.y = enemy.y + (100 * dt)
		if enemy.y > 600 then
-- TAREFA 07	
-- DESALOCAÇÃO: Neste momento os inimigos que não sofreram colisão, são desalocados após 
-- a posição do inimigo no eixo y for igual a 600.
			table.remove(enemies,i)
		end
	end	
end
